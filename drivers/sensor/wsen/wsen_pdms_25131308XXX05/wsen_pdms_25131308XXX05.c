					  ...
					  struct sensor_value *val)
{
	...
	switch (chan) {
	case SENSOR_CHAN_AMBIENT_TEMP: {
		int64_t milli_degree_temp =
			((int64_t)(data->temperature_data - T_MIN_TYP_VAL_PDMS)) * 4272;
		status = sensor_value_from_micro(val, milli_degree_temp);
		break;
	}
	case SENSOR_CHAN_PRESS: {
...
static int pdms_25131308XXX05_init(const struct device *dev)
{
	...
	case WE_spi: {
		...
		data->sensor_interface.options.spi.protocol = (config->crc ? 1 : 0);
...
#define PDMS_25131308XXX05_SPI_OPERATION (SPI_WORD_SET(8) | SPI_OP_MODE_MASTER)
	...
	COND_CODE_1(                                                                            \
        ...
        (.bus_cfg = {                                                                       \
            .spi = SPI_DT_SPEC_INST_GET(                                                    \
                inst, PDMS_25131308XXX05_SPI_OPERATION, 0)                                  \
        },),                                                                                \
        ()                                                                                  \
    )
...
#define PDMS_25131308XXX05_CONFIG_CRC(inst) .crc = DT_INST_PROP(inst, crc),
...
#define PDMS_25131308XXX05_DEFINE(inst)                                                            \
	...
	SENSOR_DEVICE_DT_INST_DEFINE(inst, pdms_25131308XXX05_init, NULL,                          \
				     &pdms_25131308XXX05_data_##inst,                              \
				     &pdms_25131308XXX05_config_##inst, POST_KERNEL,               \
				     CONFIG_SENSOR_INIT_PRIORITY, &pdms_25131308XXX05_driver_api)
